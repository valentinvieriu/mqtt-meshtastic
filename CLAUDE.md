...
### Algorithm
- AES-128-CTR for keys ≤ 16 bytes
- AES-256-CTR for keys > 16 bytes
- Keys expanded by repetition to 16 or 32 bytes
...
### Default Key
- Short form: `AQ==` (base64 for single byte 0x01)
- Expanded form: `1PG7OiApB1nwvP+rz05pAQ==`
...
```javascript
channelHash = xorAllBytes(channelName) ^ xorAllBytes(keyBytes)
```
...
3. Server: Attempts decryption with default key
4. Server: If success, decodes Data message
5. Server: If portnum=1, extracts UTF-8 text
6. Server: Broadcasts to all WebSocket clients with metadata
7. Browser: Displays in activity log with decryption status
...
### crypto.js
- `generateChannelHash(channelName, keyBase64)` - XOR hash for channel field
